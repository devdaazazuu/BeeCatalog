...
# Configuração do Celery para desenvolvimento local
# Para produção, altere para usar SQS com credenciais AWS configuradas
if DEBUG:
    # Modo síncrono para desenvolvimento local (sem broker externo)
    CELERY_TASK_ALWAYS_EAGER = True
    CELERY_TASK_EAGER_PROPAGATES = True
    CELERY_BROKER_URL = 'memory://'
    # Desabilita o backend de resultados no modo de desenvolvimento
    CELERY_RESULT_BACKEND = None
    CELERY_TASK_IGNORE_RESULT = True
else:
    # Configuração para produção com SQS
    CELERY_TASK_ALWAYS_EAGER = False
    CELERY_BROKER_URL = 'sqs://'
    CELERY_BROKER_TRANSPORT_OPTIONS = {
        'region': os.getenv('AWS_S3_REGION_NAME', 'sa-east-1'), 
    }
    # Backend de resultados apenas em produção
